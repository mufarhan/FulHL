...
class HighwayLabelling {
 public:
  ...
  void insert_exact(int a, int b, uint8_t *P, int *visited_vertices);
  void insert_prune(int a, int b, uint8_t *P, int *visited_vertices);
  void remove(int a, int b, uint8_t *P, uint8_t *D, int *visited_vertices, long &av);
...
void HighwayLabelling::UpdateLabelling(std::string filename) {
  ...
  while (ifs >> op >> a >> b) {
    if(op == "EI") {
      ...
      insert_prune(a, b, P, visited_vertices);
    } else if(op == "ED") {
...
void HighwayLabelling::insert_exact(int a, int b, uint8_t *P, int *visited_vertices) {
...
void HighwayLabelling::insert_prune(int a, int b, uint8_t *P, int *visited_vertices) {
  ...
  for(int i = 0; i < K; i++) {
    ...
    while (!que.empty()) {
      ...
      //  highway[i][landmarks[u]] = P[u];
      //} else {
        bool flag = false; std::vector<int> neighs;
        for (int w : adj[u]) {
          if(P[w] == 111) {
	    P[w] = query(i, w);
	    visited_vertices[c] = w; c++;
...
void HighwayLabelling::QueryDistance(std::string pairs, std::string output) {
  ...
  std::ifstream ifs(pairs); int s = 0, t = 0;
